...
// cache라는 객체 만들어 수행되는 이전 결과값 넣어두기
// 이렇게 하면 피보나치 로직 빠르게 연산됨.
/*
const cache = {}
const memoFibo = (n)=>{
  if(n <= 0) return 0;
...
// 함수지만 객체처럼 프로퍼티를 저장할 수 있다. 
// 그래서 memoFibo 객체(함수) 안에 키와 값으로 cache 만들 수 있다!
// 즉, 함수 내부에 메모리를 넣어주는 것!
// 이렇게 되면 관리에 용이하다. 
const memoFibo = (n)=>{
  if(n <= 0) return 0;
  if(n <= 2) return 1;
...
// 베열인 경우 그 배열 안의 객체의 salaries만을 찾는다.
// 우리가 찾는 대상이 객체인 경우 +가 일어남. -> if 문 사용
