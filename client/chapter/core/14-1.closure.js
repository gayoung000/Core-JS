...
// 가비지컬랙터는 함수의 실행이 끝나면 모든 메모리를 수거해 갑니다. 
// 함수의 변수를 기억하기 위해 클로저를 사용합니다. 
// 클로저란, 내부 함수가 외부 변수에 대한 참조를 할 수 있는 환경을 제공해주는 함수입니다. 

function sum(a,b){
  return a + b
}

const value = sum  // 함수 자체(본문) 담김. 
 // 그럼 value또한 함수 역할을 할 수 있기 때문에 value를 실행시킬 수 있다. 
...
// const result = first(); //result가 first()처럼 작동하는거고, 이걸 통해 second()도 실행시킬 수 있다. 
// 클로저는 함수가 함수를 내보내는게 클로저는 아님.
// 안쪽에 있는 대상이 바깥쪽 대상을 사용해야 클로저..변수 x 처럼
...
const c1 = counter(); // c1이란 실행환경 만듦
const c2 = counter(); // c2이란 실행환경 만듦
const c3 = counter(); // c3이란 실행환경 만듦
*/
